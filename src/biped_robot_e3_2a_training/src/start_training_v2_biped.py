...
if __name__ == '__main__':
    ...
    rospy.init_node('biped_gym', anonymous=True, log_level=rospy.INFO)

    # Create the Gym environment
    env = gym.make('Biped-v0')
    rospy.logdebug ( "Gym environment done")
    reward_pub = rospy.Publisher('/biped/reward', Float64, queue_size=1)
    episode_reward_pub = rospy.Publisher('/biped/episode_reward', Float64, queue_size=1)

    # Set the logging system
    rospack = rospkg.RosPack()
    pkg_path = rospack.get_path('biped_robot_e3_2a_training')

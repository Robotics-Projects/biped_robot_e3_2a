...
"""
 wrenches:
  ...
  -
    info: "Debug:  i:(2/4)     my geom:biped::lowerleg::lowerleg_contactsensor_link_collision_1\
  \   other geom:ground_plane::link::collision         time:50.405000000\n"
    collision1_name: "biped::lowerleg::lowerleg_contactsensor_link_collision_1"
...
class BipedState(object):
    ...
    def __init__(self, max_height, min_height, abs_max_roll, abs_max_pitch, joint_increment_value = 0.05, done_reward = -1000.0, alive_reward=10.0, desired_force=7.08, desired_yaw=0.0, weight_r1=1.0, weight_r2=1.0, weight_r3=1.0, weight_r4=1.0, weight_r5=1.0, discrete_division=10):
        rospy.logdebug("Starting bipedState Class object...")
        ...
        rospy.Subscriber("/L_lowerleg_contactsensor_state", ContactsState, self.contact_callback)
        # We use it to get the contact force, to know if its in the air or stumping too hard.
        rospy.Subscriber("/R_lowerleg_contactsensor_state", ContactsState, self.contact_callback)
    ...
    def check_all_systems_ready(self):
        ...
        while imu_data is None and not rospy.is_shutdown():
            try:
                imu_data = rospy.wait_for_message("/biped/imu/data", Imu, timeout=0.1)
        ...
        L_contacts_data = None
        while L_contacts_data is None and not rospy.is_shutdown():
            try:
                L_contacts_data = rospy.wait_for_message("/L_lowerleg_contactsensor_state", ContactsState, timeout=0.1)
                for state in L_contacts_data.states:
        ...
        R_contacts_data = None
        while R_contacts_data is None and not rospy.is_shutdown():
            try:
                R_contacts_data = rospy.wait_for_message("/R_lowerleg_contactsensor_state", ContactsState, timeout=0.1)
                for state in R_contacts_data.states:
        ...
        while joint_states_msg is None and not rospy.is_shutdown():
            try:
                joint_states_msg = rospy.wait_for_message("/biped/joint_states", JointState, timeout=0.1)
    ...
    def set_desired_world_point(self, x, y, z):
        """
        Point where you want the biped to be
        :return:
        """
    ...
    def get_contact_force_magnitude(self):
        ...
        :return:
        """
        contact_force = Vector3(x=self.L_contact_force.x+self.R_contact_force.x, 
            y = self.L_contact_force.y + self.R_contact_force.y,
            z = self.L_contact_force.z+self.R_contact_force.z)
    ...
    def get_observations(self):
        ...
        L_joint_states_haa = joint_states.position[0]
        L_joint_states_hfe = joint_states.position[1]
        L_joint_states_kfe = joint_states.position[2]
        R_joint_states_haa = joint_states.position[3]
        R_joint_states_hfe = joint_states.position[4]
        R_joint_states_kfe = joint_states.position[5]
        ...
        for obs_name in self._list_of_observations:
            ...
            elif obs_name == "L_joint_states_haa":
                observation.append(L_joint_states_haa)
            elif obs_name == "L_joint_states_hfe":
                observation.append(L_joint_states_hfe)
            elif obs_name == "L_joint_states_kfe":
                observation.append(L_joint_states_kfe)
            elif obs_name == "R_joint_states_haa":
                observation.append(R_joint_states_haa)
            elif obs_name == "R_joint_states_hfe":
                observation.append(R_joint_states_hfe)
            elif obs_name == "R_joint_states_kfe":
                observation.append(R_joint_states_kfe)
            else:
    ...
    def get_action_to_position(self, action):
        ...
        if action == 0: #Increment L_haa_joint
        ...
        elif action == 1: #Decrement L_haa_joint
            action_position[0] = joint_states_position[0] - self._joint_increment_value
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 2: #Increment L_hfe_joint
            ...
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 3: #Decrement L_hfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1] - self._joint_increment_value
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 4: #Increment L_kfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            ...
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 5:  #Decrement L_kfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2] - self._joint_increment_value
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        if action == 6: #Increment R_haa_joint
            action_position[0] = joint_states_position[0] 
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            ...
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 7: #Decrement R_haa_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3] - self._joint_increment_value
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 8: #Increment R_hfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            ...
            action_position[5] = joint_states_position[5]
        elif action == 9: #Decrement R_hfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4] - self._joint_increment_value
            action_position[5] = joint_states_position[5]
        elif action == 10: #Increment R_kfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
        ...
        elif action == 11:  #Decrement R_kfe_joint
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
    ...
    def process_data(self):
        ...
        :return: reward, done
        """
        biped_height_ok = self.biped_height_ok()
        biped_orientation_ok = self.biped_orientation_ok()

        done = not(biped_height_ok and biped_orientation_ok)
        if done:
...
if __name__ == "__main__":
    rospy.init_node('biped_state_node', anonymous=True)
    biped_state = BipedState(max_height=3.0,
                                 min_height=0.6,
                                 abs_max_roll=0.7,
                                 abs_max_pitch=0.7)
    biped_state.testing_loop()
